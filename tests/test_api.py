...
from config import (BAD_REQUEST, CLIENT_BUSIK, CLIENT_STESHA, CREATED,
                    FORBIDDEN, METOD_NOT_ALLOWED, NO_CONTENT, OK, TEST_TITLE)
...
class BankAccountAPITest(TestCase):
    def setUp(self):
        ...
        self.bank = Bank.objects.create(
            title=TEST_TITLE, foundation_date=timezone.now().date())
        # creating client
    ...
    def test_bank_account_creation(self):
        ...
        self.api_client.force_authenticate(
            user=self.superuser, token=self.superuser_token)
        body = {'balance': 1000.00, 'bank': f'/api/bank/{self.bank.id}/',
        ...
        bank_account_id = url.removeprefix(
            '/api/bank_account/').removesuffix('/')
...
class TransactionAPITest(TestCase):
    ...
    def test_transaction_update(self):
        ...
        transaction_id = url.removeprefix(
            '/api/transaction/').removesuffix('/')
